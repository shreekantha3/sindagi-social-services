# Social services — Website (Sindagi)

- **Live:** https://shreekantha3.github.io/sindagi-social-services/
- **Repo:** `shreekantha3/sindagi-social-services` (branch `main` = source, `gh-pages` = deployed bundle)
- **Stack:** static Vite + Tailwind CSS, no backend. `base: '/sindagi-social-services/'` for GitHub Pages project-site hosting.
...
- **Rebuild + publish:** `npm ci && npm run build`, commit, push `main`, refresh the `gh-pages` branch with `dist/` (see `scripts/deploy_site.sh`).
